...
import datetime #to import the library datetime which will help in calculating time taken from sending the request until we receive the response
...
while True: #infinite loop 
    ...
    if not url.startswith('http'): #this will check if the entered link starts with http or not
        url = 'http://' + url #if it did not start with http, it will add <http://> at the beginning of the link to avoid problems
    ...
    url = requests.head(url) #using head method  to request the headers that would be returned if the head request's url was instead requested with the http get method 
