...
SERVER = '127.0.0.1' #Giving our server an ip address
ADDR = (SERVER, PORT)
FORMAT = 'utf-8'
    ...
    while True:
        ...
        try:
            if myfile == '': #If nothing was send with the request, the default is the main html file
                myfile = 'index.html'
            elif myfile.lower() == 'sortbyname' or myfile.lower() == 'sortbyprice': #It will convert the request into lower case
                old_myfile = myfile
                myfile = 'items.txt'
                with open('input.txt', 'r+') as file, open(myfile, 'w') as outfile: #It will deal with input file as an input file, and items file as an output file
                    ...
                    for data in arr: #For loop to print the sorted data into a new file <items.txt>
        ...
        except FileNotFoundError: #The exception that will be sent if the requested order was not found
            conn.send(f"Content-Type: text/html \r\n".encode(FORMAT)) 
